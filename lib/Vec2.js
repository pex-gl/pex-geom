...
Vec2.fromAngle = function(angle, dist) {
  return new Vec2().setAngle(angle, dist);
}
...
Vec2.prototype.setAngle = function(angle, dist) {
  dist = dist || 1;

  this.x = dist * Math.sin(angle / 360 * Math.PI * 2);
  this.y = dist * Math.cos(angle / 360 * Math.PI * 2);
